...
server_ip_list="192.168.0.71 192.168.0.72"
server_count=2
server_user="root"
server_passwd="xxxx"
...
run_test()
{
	...
	do
		...
		do
			...
			if [ $test_time -lt $test_timeout ];then
				...
				#store result,execute on FS(cann't connect to MySQL on test VM)
				expect -f ../scp_file.exp "send" "$fs_ip" "$fs_user" "$fs_passwd" "$result_file" "$fs_data_path"
				expect -f ../ssh_cmd.exp "$fs_ip" "cd $fs_work_path;java -jar perf_report.jar -s $fs_data_path/$result_file"  "$fs_user" "$fs_passwd" "30"
			fi
			break;
		done
	done
	...
	#remove data on FS
	expect -f ../ssh_cmd.exp "$fs_ip" "rm -fr $fs_data_path/*xml" "$fs_user" "$fs_passwd" "10"
}

print_usage()
{
	echo "$1 is test model,$2 is single test like I or R"
}
...
#generate XML
expect -f ../ssh_cmd.exp "$fs_ip" "cd $fs_work_path;java -jar perf_report.jar -g $run_time"  "$fs_user" "$fs_passwd" "120"
